...
class SensorService {
   ...
   */
  getConnectionStatus() {
    return derived([this.isConnected, this.connectionStatus], ([$connected, $status]) => ({
      connected: $connected,
      status: $status,
    }))
  }
   ...
   */
  async configureSensorSource(sourceId: string, config: Partial<SourceConfiguration>): Promise<void> {
    try {
      const response = await fetch(`/api/sensors/sources/${sourceId}/configure`, {
        method: "POST",
      ...
      if (!response.ok) {
        throw new Error(`Failed to configure sensor source: ${response.statusText}`)
      }

      console.log(`[SensorService] Configured sensor source ${sourceId}`)
      await this.discoverSensorSources() // Refresh sources
    } catch (error) {
      console.error(`[SensorService] Failed to configure source ${sourceId}:`, error)
      throw error
    }
  }
  ...
  async toggleSensorSource(sourceId: string, enabled: boolean): Promise<void> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to toggle sensor source: ${response.statusText}`)
      }

      console.log(`[SensorService] ${enabled ? "Enabled" : "Disabled"} sensor source ${sourceId}`)
      await this.discoverSensorSources() // Refresh sources
    } catch (error) {
      console.error(`[SensorService] Failed to toggle source ${sourceId}:`, error)
      throw error
    }
  }
  ...
  private async loadConfiguration(): Promise<void> {
    try {
      ...
      console.warn("[SensorService] Failed to load configuration, using defaults:", error)
    }
  }

  private async saveConfiguration(): Promise<void> {
    try {
      localStorage.setItem("sensor-realtime-config", JSON.stringify(this.config))
    } catch (error) {
  ...
  private processSensorDataMessage(message: WebSocketSensorMessage): void {
    if (message.data && message.data.sources) {
      ...
      for (const [sourceId, sourceData] of Object.entries(message.data.sources)) {
        if (sourceData.active && sourceData.sensors) {
          for (const [sensorId, sensorData] of Object.entries(sourceData.sensors)) {
            batch.sensors[sensorId] = this.normalizeSensorReading(sensorData, sourceId)
          }
        }
      }
  ...
  private processSensorUpdate(message: WebSocketSensorMessage): void {
    ...
    if (message.data && message.data.sensor_id) {
      const reading = this.normalizeSensorReading(message.data, message.source_id)
  ...
  private assessDataQuality(data: any): "excellent" | "good" | "fair" | "poor" | "unknown" {
  ...
  private async discoverSensorSources(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to discover sensor sources: ${response.statusText}`)
      }
      ...
      if (data.sources) {
        ...
        for (const [sourceId, sourceData] of Object.entries(data.sources)) {
          normalizedSources[sourceId] = this.normalizeSensorSource(sourceId, sourceData)
        }

        this.sources.set(normalizedSources)
        console.log("[SensorService] Discovered sensor sources:", Object.keys(normalizedSources))
      }
    } catch (error) {
      console.error("[SensorService] Failed to discover sensor sources:", error)
      throw error
    }
  }
    ...
    try {
      ...
      if (data.sources) {
        ...
        for (const [sourceId, sourceData] of Object.entries(data.sources)) {
          if (sourceData.active && sourceData.sensors) {
            for (const [sensorId, sensorData] of Object.entries(sourceData.sensors)) {
              allSensors[sensorId] = this.normalizeSensorReading(sensorData, sourceId)
            }
          }
        }
  ...
  private startRealTimePolling(): void {
    ...
    if (this.config.polling_rate > 0) {
      ...
      console.log(`[SensorService] Started real-time polling at ${this.config.polling_rate}ms intervals`)
    }
  }
