...
export const sensorHistory = writable<Record<string, Array<{ timestamp: Date; value: number }>>>({})
...
export const sensorUtils = {
  ...
  // Get sensor history
  getSensorHistory: (sensorId: string, maxPoints?: number): Array<{ timestamp: Date; value: number }> => {
  ...
  checkSensorAlerts: (sensorId: string): { level: "normal" | "warning" | "critical"; message?: string } => {
    const sensor = sensorUtils.getSensorById(sensorId)
    const metadata = get(sensorMetadata)[sensorId]
    ...
    if (sensor.value >= critical) {
      return { level: "critical", message: `Critical: ${sensor.value}${sensor.unit} >= ${critical}${sensor.unit}` }
    }

    if (sensor.value >= warning) {
      return { level: "warning", message: `Warning: ${sensor.value}${sensor.unit} >= ${warning}${sensor.unit}` }
    }
