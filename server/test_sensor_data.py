...
def test_sensor_endpoints():
    ...
    try:
        ...
        expected_categories = ['Temperature', 'Usage', 'Clock', 'Voltage', 'Fan', 'Data', 'Load']
        ...
        try:
            ...
            ws = websocket.WebSocketApp(f"ws://127.0.0.1:8100/ws",
...
def test_sensor_apis():
    ...
    for endpoint in endpoints:
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                print(f"✅ {endpoint}: OK ({len(data) if isinstance(data, (list, dict)) else 'data'} items)")
            else:
