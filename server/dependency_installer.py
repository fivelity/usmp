...
class DependencyInstaller:
        ...
        for package in packages:
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.results["failures"].append(f"Failed to install {package}: {result.stderr}")
    ...
    def _fix_dotnet_issues(self):
        ...
        for version in pythonnet_versions:
            try:
                ...
                result = subprocess.run(
                    [sys.executable, "-m", "pip", "install", f"pythonnet=={version}", "--force-reinstall"],
                    capture_output=True,
                    text=True,
                    timeout=300
                )
                ...
                if result.returncode == 0:
                    ...
                    self.results["actions_taken"].append(f"Installed pythonnet {version}")
        ...
        if not dll_path.exists():
            ...
            for url in dll_urls:
                try:
                    ...
                    if dll_path.exists() and dll_path.stat().st_size > 100000:  # At least 100KB
                        print(f"   ✅ LibreHardwareMonitorLib.dll downloaded successfully")
                        print(f"      Size: {dll_path.stat().st_size:,} bytes")
                        self.results["actions_taken"].append("Downloaded LibreHardwareMonitorLib.dll")
                        break
                    else:
            ...
            if not dll_path.exists():
                ...
                print("      1. Go to https://github.com/LibreHardwareMonitor/LibreHardwareMonitor/releases")
    ...
    def _install_hardware_monitor_package(self):
        ...
        for package_name, display_name in packages_to_try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.results["actions_taken"].append(f"Installed {package_name} via pip")
            ...
            print(f"      Method 2: Alternative installation methods for {package_name}...")
            ...
            for cmd in alternative_commands:
                try:
                    ...
                    if result.returncode == 0 and self._test_hardware_monitor(package_name):
                        print(f"   ✅ {package_name} installed successfully")
                        self.results["actions_taken"].append(f"Installed {package_name} with alternative method")
                        return
                except Exception:
                    continue
        ...
        print("      1. Ensure you're running as Administrator (REQUIRED for hardware access)")
        ...
        print("      6. Based on PyHardwareMonitor docs: Admin privileges are mandatory!")
        
        self.results["warnings"].append("Manual HardwareMonitor installation may be required")
