...
async def list_sensors(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve sensor definitions")
...
async def get_sensor_definition(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Sensor definition {sensor_id} not found")
        
    except HTTPException:
        raise  # Re-raise HTTP exceptions
    except Exception as e:
        logger.error(f"Error retrieving sensor definition for {sensor_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to retrieve sensor definition")
...
async def get_sensor_data(
    ...
    limit: Optional[int] = Query(10, description="Limit the number of recent readings", ge=1, le=100),
    sensor_manager: SensorManager = Depends(get_sensor_manager)
) -> List[SensorReading]:
    """
    ...
    try:
        ...
        if not sensor_readings:
            logger.info("No data available for sensor_id: %s. Returning empty list.", sensor_id)
            return []
        
        # Sort by timestamp descending to get the most recent readings
        sorted_readings = sorted(sensor_readings, key=lambda r: r.timestamp, reverse=True)
        
        return sorted_readings[:limit]
        
    except Exception as e:
        logger.error(f"Error retrieving sensor data for {sensor_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to retrieve sensor data")
