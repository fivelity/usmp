...
class RealTimeService:
    ...
    def __init__(self, sensor_manager: SensorManager, websocket_manager: WebSocketManager):
    ...
    async def start(self, app_settings: AppSettings) -> None:
        ...
        self.broadcast_interval = getattr(app_settings, 'realtime_broadcast_interval', 2.0)
        
        self.logger.info(f"Starting RealTimeService with {self.broadcast_interval}s interval")
        ...
        while self.is_running:
            try:
                ...
                if sensor_data:
                    ...
                    for source_id, readings in sensor_data.items():
                        serializable_data[source_id] = [
                            reading.model_dump(mode='json') if hasattr(reading, 'model_dump') else reading.dict()
                    ...
                    broadcast_data = {
                        ...
                        "active_sources": len([source for source, readings in sensor_data.items() if readings])
                    }
                    ...
                    if self.broadcasts_sent % 30 == 0:  # Log every 30 broadcasts (1 minute at 2s interval)
                        self.logger.debug(
                ...
                if self.errors_count > 10:
                    self.logger.warning("Too many broadcast errors, increasing interval")
    ...
    async def force_broadcast(self) -> bool:
        ...
        try:
            self.logger.info(f"🚀 Force broadcast starting - connected clients: {len(self.websocket_manager.active_connections)}")
            ...
            if sensor_data:
                # Convert SensorReading objects to dictionaries for JSON serialization
                self.logger.info("🔄 Converting sensor data to JSON-serializable format...")
                serializable_data = {}
                for source_id, readings in sensor_data.items():
                    serializable_data[source_id] = [
                        reading.model_dump(mode='json') if hasattr(reading, 'model_dump') else reading.dict()
                    ...
                    self.logger.info(f"   Converted {len(readings)} readings from {source_id}")
                ...
                self.logger.info(f"📡 Broadcasting {total_sensors} sensors from {active_sources} sources...")
                await self.websocket_manager.broadcast_sensor_data(broadcast_data)
