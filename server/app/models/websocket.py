...
class SensorDataMessage(WebSocketMessage):
    ...
    quality_score: Optional[float] = Field(
        None, ge=0, le=100, description="Data quality score"
    )
...
class ConfigurationMessage(WebSocketMessage):
    ...
    type: MessageType = Field(
        MessageType.CONFIGURE_REALTIME, description="Message type"
    )
    ...
    applied_at: Optional[datetime] = Field(
        None, description="Configuration application time"
    )
