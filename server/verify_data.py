...
def verify_sensor_data():
    try:
        ...
        extended_indicators = {
            'Voltage sensors': len([s for s in data.values() if s.get('category') == 'Voltage']),
            'Fan sensors': len([s for s in data.values() if s.get('category') == 'Fan']),
            'Clock sensors': len([s for s in data.values() if s.get('category') == 'Clock']),
            'Data sensors': len([s for s in data.values() if s.get('category') == 'Data']),
            'Temperature sensors': len([s for s in data.values() if s.get('category') == 'Temperature']),
